...
#' @return A table of \code{x} by \code{g} (invisible)
#' @examples
#' # Use \code{pyramid.plot()} on anthropometric data from a SMART survey in
...
pyramid.plot <- function(x,
                         ...
                         xlab = paste(deparse(substitute(g)), "(", levels(as.factor(g))[1], "/", levels(as.factor(g))[2],")"),
